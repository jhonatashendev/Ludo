...
const BoardBackground = React.memo(({ getPlayerName }: { getPlayerName: (color: string) => string }) => (
  <>
    <g id="tracks">
...
export const Board2D = React.memo(function Board2D() {
  ...
  // Memoize player names so BoardBackground doesn't re-render unless players change
  const playersStr = JSON.stringify(room?.players?.map((p: any) => ({ color: p.color, name: p.name })));
  const getPlayerName = React.useCallback((color: string) => {
    const player = room?.players?.find((p: any) => p.color === color);
    return player ? player.name.substring(0, 8) : '';
  }, [playersStr, room?.players]);
  ...
  return (
    <div className="w-full h-full bg-[#1e1b4b] rounded-[32px] shadow-[0_0_40px_rgba(88,28,255,0.4)] p-3 md:p-5 relative transition-transform ring-4 ring-[#1e1b4b]">
       <svg viewBox="0 0 15 15" className="w-full h-full drop-shadow-xl z-10 relative bg-white rounded-xl overflow-hidden shadow-inner flex" style={{ stroke: colors.BORDER, strokeWidth: 0.05, strokeLinejoin: 'round' }}>
         <BoardBackground getPlayerName={getPlayerName} />
